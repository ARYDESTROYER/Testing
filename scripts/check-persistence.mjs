...
const sync = await j(await fetch(`${BASE}/api/session/${id}/sync`, {
  ...
  body: JSON.stringify({
    attributes: [
      { ...base, id: 'k1', text: 'kept one', side: 'ys', x: 100, y: 100 },
      { ...base, id: 't1', text: 'handed over', side: 'ds', x: 200, y: 200, transferredAt: 300, transferredBy: 'participant' },
      { ...base, id: 'g1', text: 'let go of', side: 'gone', x: 300, y: 300, transferredAt: 400, transferredBy: 'participant' },
      { ...base, id: 'bad', text: 'nowhere', side: 'elsewhere', x: 0, y: 0 },
    ],
    events: [{ type: 'attribute_discarded', at: 400, payload: { id: 'g1' } }],
  }),
}))
console.log('sync ->', JSON.stringify(sync))

const one = await j(await fetch(`${BASE}/api/export?session=${encodeURIComponent(id)}`))
const sides = Object.fromEntries(one.attributes.map((a) => [a.id, a.side]))
console.log('stored sides ->', JSON.stringify(sides))

const csv = await (await fetch(`${BASE}/api/export?format=csv`)).text()
const rows = csv.split('\r\n').filter((l) => l.includes('let go of'))
